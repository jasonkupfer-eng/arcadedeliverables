...
// MASSIVE BRIGHTNESS BOOST during the final banner hold
            let starBright = (cinematicState === 'done') ? 0.8 + Math.sin(s.a) * 0.5 : 0.4 + Math.sin(s.a) * 0.5;
            mgctx.fillStyle = `rgba(255, 255, 255, ${Math.min(1, starBright)})`; 
...
// --- BULLETPROOF HUD COUNTER & BOSS METER ---
        if (gameState === 'playing' && !cinematicState) {
            ...
            mgctx.shadowBlur = 0; 
            mgctx.fillText("BOTS BLASTED: " + String(sessionKills).padStart(3, '0'), 20, 28);
