    ...
    function gameLoop() {
        ...
        if (gameState === 'playing') {
            ...
            if (sessionKills >= 15 && !boss && !cinematicState) {
                ...
                bots = [];
                boss = { x: player.x, y: -200, vx: 8, vy: 0, hp: 10, state: 'falling', textTimer: 50 };
            }
            ...
            if (boss && boss.hp > 0) {
                ...
                // Check if all terminals are destroyed for Phase 2
                let allSmashed = terminals.every(t => t.smashed);
                let bounceFloor = allSmashed ? GROUND_Y - 20 : GROUND_Y - 120;

                // Grow arms dynamically when in Phase 2
                if (allSmashed && boss.armExt < 1) boss.armExt += 0.05 * timeScale;

                // Ground Bounce & Terminal Smashing
                if (boss.y > bounceFloor) {
                    boss.y = bounceFloor;
                    boss.vy = -18; 
                    SFX.explosion();
                    createExplosion(boss.x, GROUND_Y, '#ff007f');
        ...
        // Starry Sky & Planets (Fades out slowly at the end)
        mgctx.globalAlpha = skyOpacity;
        mgctx.save(); mgctx.translate(camera.x * 0.8, camera.y * 0.8); // Parallax offset
        stars.forEach(s => { 
            s.a += 0.05 * timeScale; 
            
            // THE FIX: Boosted the base opacity to 0.6 and the twinkle multiplier to 0.6. 
            // This forces the stars to peak at 100% solid white and prevents them from fading out completely.
            mgctx.fillStyle = `rgba(255, 255, 255, ${0.6 + Math.sin(s.a) * 0.6})`; 
        ...
        if (boss && boss.hp > 0) {
            ...
            // Health Bar
            mgctx.fillStyle = '#111'; mgctx.fillRect(-50, -220 + bossBob, 100, 10);
            mgctx.fillStyle = '#ff007f'; mgctx.fillRect(-50, -220 + bossBob, (boss.hp/10)*100, 10);
            mgctx.restore();
        }
...
// --- BULLETPROOF HUD COUNTER ---
        if (gameState === 'playing' && !cinematicState) {
            // 1. Force-reset the canvas transform to guarantee top-left screen-space coordinates
            mgctx.setTransform(1, 0, 0, 1, 0, 0);
            
            // 2. Draw a dark backing plate to guarantee the text pops against the background
            mgctx.fillStyle = "rgba(0, 5, 0, 0.8)";
            mgctx.fillRect(10, 10, 250, 36);
            mgctx.strokeStyle = "#39ff14";
            mgctx.lineWidth = 2;
            mgctx.strokeRect(10, 10, 250, 36);
            
            // 3. Draw the text with a robust system fallback font
            mgctx.fillStyle = "#39ff14";
            mgctx.font = "14px 'Press Start 2P', 'Courier New', monospace";
            mgctx.textAlign = "left";
            mgctx.textBaseline = "middle";
            mgctx.shadowBlur = 0; // Ensure no rogue shadows from previous draw calls blur the text out
